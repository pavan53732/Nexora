...
The `WorkflowEngine` class manages the graph traversal and step dispatching. It uses a topological sort to determine execution order (treating iteration edges specially) and maintains an in-memory `StepStatusMap` that is a **cache over the persisted `workflow_step` rows**, never an independent authority: every step status and `iterationIndex` change is persisted to the `workflow_step` table before the corresponding event is published, per the Normative Transition Contract above. On restart, persisted `workflow` and `workflow_step` state (including `iterationIndex` for iterative steps) is authoritative, and incomplete work resumes only through the listed recovery transitions. The engine is coroutine-based — each `stepStart()` launches a child job so that independent parallel branches execute concurrently, with structured concurrency ensuring cancellation propagates to all children.
