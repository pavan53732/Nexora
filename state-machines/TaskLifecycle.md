...
A runtime, activity-feed, or coordination projection MAY expose the existing `taskId`, `workspaceId`, assigned `agentId`, parent/child lineage, `Execution`/`Workflow`/`PlanStep` references when applicable, version, checkpoint reference, heartbeat/freshness, immutable effective deadline and remaining budget, blocker, recovery/replan candidate, evidence references, and final disposition. The projection is observational and coordinating only; TaskLifecycle remains the sole authority for Task state and legal transitions.

The projection MUST NOT adopt or reparent a Task, create or select a lifecycle transition, replay an uncertain Tool operation, reset a deadline/retry budget, authorize a permission or Tool call, or trigger an autonomous side effect. Missing or stale values MUST remain unknown or unavailable and MUST be resolved through the existing checkpoint, retry, escalation, cancellation, expiry, and failure contracts. No Task state, identity, lease, supervisor, or recovery state is introduced.
