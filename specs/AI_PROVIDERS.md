...
> **Autonomy control (`FR-S016`):** Real-time device I/O uses the automatically selected effective autonomy mode and remains subject to the existing `device:*` PermissionModel defaults and high-risk gates. `Autopilot` does not bypass `DENY`, `ASK`, explicit user grants, sandbox, Android lifecycle, or audit requirements; no per-session mode confirmation is required merely because the mode was selected.
> **Security (`security/PermissionModel.md` — deny-by-default, DEC-42):** `device:camera` and `device:microphone` remain `DENY` by default; user must explicitly grant (`ALLOW`) through workspace settings (`FR-W005`) or agent-level override. No local AI classifier is invoked under the active PermissionModel/DEC-44 boundary; existing scope, ASK, DENY, and audit controls remain authoritative.
