...
| Escalation / missing capability / clarification / exhausted unknown completion | Suspend at `BlockedAwaitingInput` gate (TaskLifecycle `requestEscalation`); for exhausted `UNKNOWN_COMPLETION`, preserve the unresolved child and reconciliation context in the checkpoint, retain the associated Execution's existing `RUNNING` status as non-terminal and resumable, and prohibit further Tool execution or automatic replay; resume on user input (`resolveEscalation`) from the checkpoint captured at suspension (same `executionId`; `version` increment per RUNTIME.md §ExecutionStatus Lifecycle). Effective deadline expiry returns `NXR-1016` and transitions the Task to `Failed`. |
