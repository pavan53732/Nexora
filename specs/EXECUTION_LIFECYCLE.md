...
| Coordinator-incarnation loss / recoverable interruption | Use the existing process-death/checkpoint recovery path for the original coordinator Task and Execution identities; preserve the immutable inherited effective deadline and resume from a valid checkpoint without adoption, reparenting, deadline reset, or new identity. If the parent is invalidated or the deadline expires, use the existing child cancellation/termination path. |
| Escalation / missing capability / clarification / exhausted unknown completion | Use `BlockedAwaitingInput` only when TaskLifecycle `requestEscalation` is explicitly required for clarification or a capability gap. For exhausted `UNKNOWN_COMPLETION`, preserve the unresolved child and reconciliation context in the checkpoint, retain evidence, apply the existing `Task Running → Failed` and `ExecutionStatus RUNNING → FAILED` effects automatically, and prohibit further Tool execution or automatic replay; no human escalation is required and the child is not converted to confirmed success or failure. Effective deadline expiry for an explicit `BlockedAwaitingInput` path returns `NXR-1016` and transitions the Task to `Failed`. |
