...
When a parent is missing, terminal, cancelled, or expired, cleanup MUST use the existing TerminalSession termination/checkpoint path and preserve the parent/session/process identities, effective deadline, correlation ID, process outcome, and audit disposition. Successful cleanup commits the existing `Closed` or `Failed` terminal outcome and releases resources. If process termination or orphan removal fails, the session MUST NOT be reattached or treated as successful; the cleanup failure, remaining process/session reference, and bounded retry/deadline information MUST be persisted. After bounded cleanup/termination attempts are exhausted, or completion cannot be proven, the TerminalSession MUST commit the existing `Failed` state and surface the derived user-visible disposition `Manual cleanup required`. The session MUST NOT be reattached, resumed, replayed, adopted, or reparented; retained process/session references, cleanup evidence, deadline/retry data, and audit disposition remain available for authorized reconciliation. `Manual cleanup required` is not a lifecycle state, error code, identity, manager, or authority. No `Orphaned`, `Suspended`, `Restored`, or cleanup lifecycle state is created.
...
A temporary terminal grant does not create a new Tool, permission scope, agent type, lifecycle state, or permanent capability. It MUST still pass the agent matrix dispatch check, `sandbox:execute`, applicable `sandbox:read`/`sandbox:write`, workspace policy, PermissionModel approval, schema, sandbox, timeout, output-cap, and resource gates. It does not authorize host paths, unrestricted network/device access, plugin/MCP access, or sensitive-app interaction. No local classifier is invoked.
