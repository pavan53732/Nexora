...
Nexora ensures background execution resilience by projecting execution events into the Android system environment:

1. **Task-State Projection:** Active background tasks project their state into the Android `ForegroundService` and `WorkManager` environments. High-priority tasks utilize Android's expedited scheduling capabilities to ensure progress during system resource constraints.
2. **System-Vital Awareness:** The runtime monitors Android system vitals (battery, thermal, and power states). Resource-intensive operations are throttled or checkpointed when system vitals indicate pressure, ensuring task durability and device stability.
...
- The agent loop publishes **`TaskProgress`** events on the event bus at each
  ...
  (FR-A010, FR-P009).
- The progress projection MUST retain the existing `workspaceId`, `taskId`,
  `executionId`, `agentId`, `correlationId`, workflow/plan-step identity when applicable,
  checkpoint/version, and evidence references. It MUST expose the current phase/action,
  elapsed or active time, latest heartbeat/freshness observation, immutable deadline and
  remaining budget, resource/concurrency condition, blocker or uncertainty, next safe
  action, verification state, and final disposition. Missing values are unknown or
  unavailable; they MUST NOT be inferred from logs, elapsed time, or provider confidence.
- The UI renders progress via `TaskCard` (progress indicator) and `ActivityCard`
  ...
  Android notifications and WorkManager results MUST preserve the same durable lineage and
...
- **Autonomy mode forced to `Manual`** (`FR-S016` — degraded Android conditions prevent trust-based elevation; existing PermissionModel `ASK`/`DENY`, high-risk approval, notification, checkpoint, and user-guidance rules remain authoritative. Low-risk declared `ALLOW` scopes retain the PermissionModel default, but degraded scheduling and task-priority restrictions still apply).
