...
- `E2E-ESC-006`: revocation or bounded-reconciliation exhaustion during an active subprocess, PTY, provider call, or background child preserves `UNKNOWN_COMPLETION`; exhausted automatic reconciliation retains checkpoint/audit lineage and applies the existing Task `Failed` plus Execution `FAILED` effects without converting the child to confirmed success or failure, human escalation, Tool execution, or automatic replay.
