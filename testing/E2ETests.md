...
- `E2E-ESC-003`: a task-scoped Terminal grant passes matrix, PermissionModel authorization, approval, sandbox, schema, timeout, output-cap, resource, and audit gates; the grant cannot be reused by another task or agent.
...
- `E2E-ESC-006`: revocation or human-reconciliation escalation during an active subprocess, PTY, provider call, or background child preserves `UNKNOWN_COMPLETION`; exhausted reconciliation places the Task in `BlockedAwaitingInput`, retains the associated Execution's existing non-terminal/resumable `RUNNING` status only, requires no Tool execution or automatic replay while blocked, preserves checkpoint/audit lineage, and uses existing expiry to `Failed`.
