...
| SEC-SBX-001 | SEC-SBX | Validate sandbox escape resistance, Android app-private storage/overlay integrity, and unrestricted guest package installation from reachable registries without pre-approval; every installation remains audited and subject to process, storage, network, rootfs/overlay, quarantine, and Android resource quotas, while host-JVM Plugin installation retains manifest/user-review controls; diagnostic inputs for ABI/assets, mount/proot readiness, storage/quota, rootfs/overlay integrity, and egress fail closed through existing paths | Security + Sandbox | Planned | `evidence/security/SEC-SBX-001/` | 2026-08-04 |
...
| SEC-DOS-001 | SEC-DOS | Validate process spawn limiting and Doze handoffs preserve source identities, checkpoint/version/evidence lineage, heartbeat/deadline reporting, notification/activity classification, resource limits, Android ABI/storage/permission/scheduling readiness, and fail-closed behavior on applicable Android devices/emulators; no silent replay, deadline reset, permission bypass, or repair authority | Security + Sandbox | Planned | `evidence/security/SEC-DOS-001/` | 2026-08-05 |
...
| SEC-NET-001 | SEC-NET | Validate host egress-proxy mediation: `AUTOPILOT` public `network:http`/`network:websocket` admission without per-workspace allowlist enrollment, `ASSISTED` opt-in behavior, HTTP port 80 support, direct-socket and inbound-listener denial, loopback/app-private floors, secret-material blocking to undeclared endpoints, audit retention, and fail-closed pinned/foreign-certificate handling | Security | Planned | `evidence/security/SEC-NET-001/` | 2026-08-05 |
...
| SEC-PERM-041 | SEC-PERM | ASK/DENY-default satisfied scope triggers SCOPE_RISK_POLICY; DEC-46 category-level defaults do not bypass existing high-risk gates; DEC-47 sensitive-domain credential entry/transaction execution remains denied while read-only navigation/extraction is permitted | Security | Planned | `evidence/security/SEC-PERM-041/` | 2026-08-06 |
