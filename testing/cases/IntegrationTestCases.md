...
| IT-AGENT-001 | IT-AGENT | Validate agent task start with durable projection of objective, phase/action, acceptance progress, ProgressSignal reason, heartbeat/freshness, inherited deadline/remaining budget, blocker, recovery candidate, next safe action, and final result classifications (`Passed`, `Failed`, `Skipped`, `Blocked`, `Unverified`) over existing Agent/Task/Execution identities; verify the projection cannot adopt/reparent work, replay uncertain work, reset budgets, authorize permissions/Tools, or trigger autonomous side effects; separately verify the evidence state and retained `EXECUTED EVIDENCE` envelope | Agent Runtime | Planned | `evidence/integration/IT-AGENT-001/` | 2026-08-04 |
...
| IT-LC-011 | IT-LC | Non-idempotent in-flight call remains `UNKNOWN_COMPLETION` through automatic reconciliation exhaustion, is never replayed, retains checkpoint/reconciliation evidence, and applies existing parent Task `Running → Failed` plus associated Execution `RUNNING → FAILED`; no human clarification, unsafe replay, or false success/failure classification occurs | Runtime | Planned | `evidence/integration/IT-LC-011/` | 2026-08-06 |
...
| IT-CONV-005 | IT-CONV | Recover interrupted or rollback-cleanup-failed branch operation as no branch or one complete branch: after cleanup failure or unproven completion, commit existing non-success, preserve source Conversation/checkpoint and operation/lineage/error/partial-artifact/audit evidence, do not expose or promote partial BranchLineage, and continue only eligible idempotent same-operation-identity recovery or DEC-31 cleanup without manual recovery or external-side-effect reversal | Conversation/Session + BranchLineage | Planned | `evidence/integration/IT-CONV-005/` | 2026-08-12 |
...
| IT-ESC-004 | IT-ESC | Task-scoped Background escalation validates ABI/asset, mount, storage/quota, rootfs/overlay integrity, permission, checkpointability, heartbeat/freshness, immutable deadline/remaining budget, notification, cancellation, Android lifecycle, resource, and degradation prerequisites; failed/unavailable/unverified readiness fails closed without repair authority or silent replay | Background Runtime + Security | Planned | `evidence/integration/IT-ESC-004/` | 2026-08-15 |
...
| IT-LIVE-002 | IT-LIVE | NXR-2002 timeout preserves `UNKNOWN_COMPLETION`, retries only after reconciliation/idempotency authorization, and after automatic exhaustion retains evidence and applies existing Task `Failed` plus Execution `FAILED`; no human clarification, Tool execution, automatic replay, or false success/failure classification occurs | Tool + Runtime | Planned | `evidence/integration/IT-LIVE-002/` | 2026-08-15 |
...
| IT-TERM-001 | IT-TERM | Background TerminalSession inherits parent identity/deadline and reconciles cancellation, terminal parent, expiry, restart, and bounded cleanup failure without reattachment; after cleanup/termination exhaustion or unproven completion, commits existing `Failed`, retains process/session references, cleanup evidence, deadline/retry data, and audit disposition, continues automatic cleanup/reconciliation only when eligible, and permits no human cleanup action, reattachment, replay, adoption, or reparenting | Terminal + Runtime | Planned | `evidence/integration/IT-TERM-001/` | 2026-08-15 |
