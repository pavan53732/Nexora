...
| `TaskCard` / `ActivityCard` | Planned-versus-actual progress, current phase, acceptance progress, checkpoint/recovery notice, escalation, incomplete outcome, terminal result, or the derived `Manual cleanup required` disposition when an existing terminal session is `Failed` after cleanup exhaustion. The card is observational and routes any user action through the existing terminal/runtime authorization and reconciliation contracts. |
