...
| `AgentCard` / `TaskCard` / `ActivityCard` | Automatic intent-driven routing projection when applicable: user goal or intent classification, selected existing agent/skill/Tool/provider capabilities, selected-team rationale, delegated objectives, dependency status, evidence target, and omitted or unavailable capability. This is observational planning data; it MUST NOT imply authorization, successful execution, a new agent identity, or a new lifecycle. |
| `ChatBubble` / `StreamingText` | Creative output classification when applicable: verified fact, user-provided canon or instruction, creative proposal, simulation or hypothetical content, assumption, or unverified output. Provisional content remains distinct from committed content; source, provenance, freshness, and evidence references remain visible when available. |
| `ActivityCard` / `TaskCard` / `FileExplorer` | Creative artifact and revision projection when applicable: artifact reference or preview, source/version lineage, acceptance state, critique or comparison result, revision disposition, and approved share/export action status when such a surface exists. This contract does not create or imply a current share/export surface. The surface MUST NOT claim originality, licensing, copyright-free status, factual correctness, implementation, testing, or executed evidence without the corresponding evidence contract. |
| `ActivityCard` / `TaskCard` | Security-intent routing and analysis projection when applicable: authorized scope, selected existing security-capable agents, safe analysis/validation status, evidence references, residual risk, prohibited or unavailable action class, and final disposition. The projection MUST NOT present a security analysis as authorization, exploit success, or executed evidence. |
