...
A creative request MUST be represented through the existing `Task`, `Execution`, `Workflow`, `PlanStep`, `ContextSnapshot`, `Memory`, artifact, `ClaimRecord`, and evidence contracts. The contract MAY describe the creative objective, intended audience, style or format constraints, user-provided canon, source and inspiration boundaries, alternatives to explore, acceptance criteria, revision strategy, artifact destination, provenance requirements, and final disposition. These are task and workflow inputs; they do not create a Creativity Engine, CreativeTask identity, creative lifecycle, or separate creative authority.

Creative exploration MAY use existing workflow branching, bounded iterative steps, read-only investigation/review, provider capability routing, skills, tools, memory, checkpoints, and artifact promotion. Each alternative, draft, simulation, critique, or revision MUST remain attributable to its existing Task/Execution/Workflow/PlanStep and artifact identities. A work-group or presentation view MAY organize these records but MUST NOT replace their identity, persistence, lifecycle, authorization, recovery, or evidence ownership.

Creative output MUST distinguish, at minimum, verified fact, user-provided canon or instruction, creative proposal, simulation or hypothetical content, assumption, and unverified output. Canonical requirements, locked decisions, verified evidence, and user-provided canon MUST NOT be silently overwritten by generated content. Significant factual claims require the existing `ClaimRecord` and evidence binding; unsupported claims remain uncertain, qualified, or withheld under the existing Evidence & Validation Engine.

A creative completion gate MUST evaluate the declared acceptance criteria, artifact integrity, provenance, applicable safety and permission rules, and evidence state. A polished draft, provider confidence, stylistic quality, or file change alone MUST NOT establish factual correctness, implementation, testing, or executed-evidence status. A creative revision budget means only the existing Task and Workflow retry, bounded-iteration, effective-deadline, resource, repair, provider, permission, sandbox, and recovery limits applicable to the same execution lineage. It is not a new budget identity, lifecycle, scheduler, authority, or user credit/token limit. Revision, critique, and experimentation MUST stop, re-plan, use an existing eligible recovery path, or return an explicit non-success disposition when those existing bounds require it.

This contract strengthens existing agent behavior and does not authorize implementation, create a new agent type, or change the ownership declared by `docs/CANONICAL_SOURCES.md`.
