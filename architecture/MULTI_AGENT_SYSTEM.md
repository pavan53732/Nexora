...
The orchestrator MUST preserve the derived capability matrix as the ordinary dispatch boundary. A parent or specialized agent cannot grant itself Terminal, Background, Delegate, or any other unlisted capability. When a delegated plan requires execution outside the current agent's declared capabilities, the orchestrator MUST prefer delegation to an eligible agent. A temporary task-scoped escalation is permitted only through the existing permission, approval, classifier, sandbox, resource, deadline, and audit gates.
