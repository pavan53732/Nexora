...
#### Worker Liveness and Deadline Projection (ADR-0010)

For each delegated child, the existing coordination trace and activity projection MUST expose the source `agentId`, `taskId`, `executionId`, `workspaceId`, `correlationId`, inherited effective deadline, remaining budget, latest heartbeat/freshness observation, queue/active/blocked condition, applicable resource or concurrency condition, checkpoint/version reference, evidence/artifact references, and final disposition when available. Heartbeat and deadline reporting are observational/coordinating projections over existing Agent/Task/Execution history, watchdog, checkpoint, scheduler, and trace records. They do not create a worker-lease identity, lease lifecycle, requeue authority, replacement-worker authority, adoption or reparenting operation, permission-escalation authority, or autonomous side-effect authority.

A stale heartbeat MUST use the existing bounded watchdog response: inspect the authoritative execution state, checkpoint recoverable work when permitted, restart through the existing process-death/checkpoint path when valid, or escalate/cancel under existing deadline, resource, and lifecycle rules. Deadline expiry MUST use the existing delegation timeout and child cancellation/termination contract; it MUST NOT reset the inherited deadline, silently requeue, adopt, or reparent the child, replay an uncertain operation, authorize a Tool or permission escalation, or trigger an autonomous side effect. A progress view MUST report unknown or unavailable values explicitly and MUST NOT infer successful work from heartbeat presence, elapsed time, or worker text.
...
### Coordinator Incarnation Loss (GAP-005 — CLOSED)
...
The existing coordination, artifact-promotion, file-lock, workflow, Task, Execution, and scheduler owners MUST preserve the parent/child checkpoint, failure, timeout, cancellation, artifact, and audit records. After coordinator-incarnation loss, recovery MUST use the existing checkpoint/process-death reconciliation path for the same coordinator Task and Execution identities when recovery is possible; the inherited effective deadline MUST NOT be reset. This is bounded recovery of the original lineage, not adoption by another coordinator. Private overlays remain unpromoted until an authorized coordinator or existing artifact-promotion owner validates them against acceptance criteria; otherwise they are abandoned through existing cleanup without becoming a new durable lifecycle state. If the parent is invalidated or the inherited deadline expires, the child follows the existing cancellation/termination path. The parent coordination result MUST be reported as incomplete, failed, or otherwise non-successful unless the existing completion gate is satisfied. Takeover, adoption, and reparenting are explicit non-goals of the current Nexora product and architecture; no current behavior, implementation, or pending decision is created for them. A separate ADR would be required only if the creator later expands product scope to add takeover, adoption, or reparenting. No new authority is created by this contract.
