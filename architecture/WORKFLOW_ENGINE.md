...
A workflow step that produces `UNKNOWN_COMPLETION` MUST remain unresolved until the Tool System reconciliation contract resolves the side effect. The Workflow Engine MUST NOT replay, skip, or mark that step successful merely because a timeout or transport interruption occurred. When reconciliation is exhausted, the existing parent escalation contract MUST invoke `requestEscalation(question)` to transition the owning Task to `BlockedAwaitingInput`; the associated Execution retains its existing `RUNNING` status as a non-terminal/resumable projection only, and no further Tool execution or automatic replay is authorized while awaiting human reconciliation. Cancellation and deadline expiry propagate through the existing Task → Execution → child operation contracts, and recoverable workflow state is checkpointed through the existing execution/checkpoint protocol. The Workflow Engine does not choose or redefine these Task/Execution effects.
