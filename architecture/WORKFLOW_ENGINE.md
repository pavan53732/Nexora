...
A workflow step that produces `UNKNOWN_COMPLETION` MUST remain unresolved until the Tool System reconciliation contract resolves the side effect. The Workflow Engine MUST NOT replay, skip, or mark that step successful merely because a timeout or transport interruption occurred. When automatic reconciliation is exhausted, the existing Task/Execution owners MUST apply their existing non-success effects: the unresolved child remains `UNKNOWN_COMPLETION`, the owning Task transitions `Running → Failed`, and the associated Execution transitions `RUNNING → FAILED`; reconciliation evidence and checkpoint context are retained, and no further Tool execution or automatic replay of the unresolved child is authorized. No human clarification is required for this exhaustion path. Cancellation and deadline expiry propagate through the existing Task → Execution → child operation contracts, and recoverable workflow state is checkpointed through the existing execution/checkpoint protocol. The Workflow Engine does not choose or redefine these Task/Execution effects.
