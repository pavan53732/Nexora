...
| | `executeTool` | `NXR-2002` | Infrastructure | Safe (Idempotent Key) | Conditional (retry counts and reconciliation authorization) | `NO_CHANGE` for ToolStatus; after automatic reconciliation exhaustion, TaskLifecycle/Runtime apply the existing `Running → Failed` Task effect and the associated Execution applies `RUNNING → FAILED` while the unresolved ToolInvocation remains `UNKNOWN_COMPLETION` | Sandbox: kill tool process, preserve `UNKNOWN_COMPLETION` and reconciliation evidence, prevent Tool execution and automatic replay of the unresolved child, persist the existing checkpoint, and continue existing automatic retry/reconciliation only when eligible; this error metadata does not select lifecycle state |
