...
| **TM-006** | Tampering | Canonical path validation (`SandboxPolicy.md`) | `SandboxFileSystem.resolve` | Block paths with parent traversal (`../`); canonical path-escape error identity is OPEN/DEFERRED | `SEC-SBX-001` | sandbox | 6 | SEC-SBX-001 passes on API 34 emulator | Path traversal via crafted input before resolution; compensating: SELinux deny on raw FS access |
