...
| TM-002 | API key theft via memory dump or insecure storage | Provider System | Critical | Android Keystore with hardware-backed encryption; keys never in plain text in app memory | Partial |
| TM-003 | Impersonated provider response (MITM) | Provider System | High | Certificate pinning for known provider endpoints; TLS 1.3 enforcement | Partial |
| TM-004 | Spoofed inter-agent message from untrusted workspace | Agent System | Medium | Message authentication tokens per workspace; workspace ID validation on every IPC call | Partial |
...
| TM-007 | Configuration file manipulation by other apps on rooted device | Storage | High | App-private storage only (`/data/data/com.nexora.app`); no world-readable files | Partial |
...
| ID | Threat | Component | Severity | Mitigation | Status |
|----|--------|-----------|----------|------------|--------|
| TM-009 | Agent action with no audit trail | Agent Runtime | High | Every tool invocation logged with timestamp, agent ID, tool, parameters, result, and permission decision | Partial |
| TM-010 | User denies having granted a permission | Permission System | Medium | Immutable permission grant log stored in Room database with creation timestamps | Partial |
...
| ID | Threat | Component | Severity | Mitigation | Status |
|----|--------|-----------|----------|------------|--------|
| TM-012 | Memory content leaked to other apps via IPC or content provider | Memory System | Critical | No content providers exposed; all inter-process communication via bound services with signature permission | Partial |
| TM-013 | API keys exposed in Logcat | Provider System | High | ProGuard/R8 stripping; `Log.wtf` for security events only; no key material in any log statement | Partial |
| TM-014 | Provider response leaking sensitive user content to logs | Provider System | Medium | Log level gating; prompt/response bodies logged at DEBUG only, stripped in release builds | Partial |
| TM-015 | Workspace files readable by other apps (backup) | Storage | High | `allowBackup=false` in manifest; Android auto-backup exclusion rules | Partial |
...
| TM-023 | Plugin requests and receives excessive permissions | Plugin System | High | Least-privilege manifest; user reviews each scope at install; no `REQUEST_INSTALL_PACKAGES` ever granted | Partial |
| TM-024 | Agent exceeds granted permissions via tool chaining | Agent System | High | Permission check on every individual tool call, not just the first; no implicit permission inheritance across chain steps | Partial |
| TM-025 | Malicious provider response injects tool invocations | Provider System | Medium | Provider output is treated as data, not code; tool calls validated against registry before execution | Partial |
...
| TM-031 | TLS identity substitution (MITM between paired instances) | Pipe Transport | Critical | Mutual TLS 1.3 using pinned `pipeKey` certificates — no CA, no self-signed prompts; `instance:connect` is `ASK` by default; deterministic secret-material blocking on outbound payloads per `NFR-SEC-013` (`specs/PIPES.md` §5, §8; `NFR-SEC-014`) | Partial |
...
The thirteen formerly OPEN/DEFERRED rows now contain all four deferral fields. They remain `Partial`, not fully mitigated, until the existing validation cases execute and reproducible evidence is retained. The populated fields record the existing owner, target phase, measurable acceptance criterion, and residual risk with interim compensating controls; no new security authority is created.
...
| **TM-002** | Spoofing | Keystore AES hardware-backed transience (`ProviderSDK.md` / `SandboxPolicy.md`) | `SecureKeyStore` wrapper | Plaintext keys never persist on disk; unreadable via adb dumps | `SEC-SECRET-001` | provider | 7 | `SEC-SECRET-001` passes on an API 34 emulator with no plaintext provider key in app-private storage, logs, dumps, evidence, or telemetry | A compromised or rooted device may inspect process memory; compensating controls are Android Keystore protection, no secret logging, app-private storage, and provider isolation |
| **TM-003** | Spoofing | TLS 1.3 pinning (`Provider-API.md`) | `SanitizingHttpClient` | Block socket on certificate mismatch or untrusted root | `SEC-FLOW-001` | provider | 7 | `SEC-FLOW-001` passes on an API 34 emulator with certificate-mismatch and untrusted-root traffic rejected and no provider completion accepted | Endpoint or pin-rotation errors could deny legitimate traffic or expose a compromised path; compensating controls are TLS validation, external-endpoint validation, profile isolation, and fail-closed routing |
| **TM-004** | Spoofing | Workspace ID token verification (`MULTI_AGENT_SYSTEM.md`) | `EventBus` boundary | Inter-agent messages outside caller's workspace scope are dropped | `SEC-PERM-001` | runtime | 6 | `SEC-PERM-001` passes on an API 34 emulator with a cross-workspace or invalid-token inter-agent message dropped before any side effect | A compromised same-workspace component could forge messages if token integrity fails; compensating controls are workspace scoping, identity validation, PermissionModel checks, and audit logging |
...
| **TM-006** | Tampering | Canonical path validation (`SandboxPolicy.md`) | `SandboxFileSystem.resolve` | Block paths with parent traversal (`../`); Tool execution returns `NXR-2009`, Plugin execution returns `NXR-6008` | `SEC-SBX-001` | sandbox | 6 | SEC-SBX-001 passes on API 34 emulator | Path traversal via crafted input before resolution; compensating: SELinux deny on raw FS access |
| **TM-007** | Tampering | Private sandbox boundaries (`FULL_ENVIRONMENT.md`) | Android OS File System | Non-root processes or other apps cannot read app-private data | `SEC-SBX-001` | sandbox | 6 | `SEC-SBX-001` passes on an API 34 emulator and another app or non-root process cannot read or modify app-private workspace data or overlays | A rooted or compromised Android OS may bypass app-private boundaries; compensating controls are Android OS/SELinux enforcement, app-private storage, integrity verification, and fail-closed Sandbox behavior |
...
| **TM-009** | Repudiation | Mandatory audit logging (`SandboxPolicy.md`) | `ToolManager.execute` | Every tool execution is recorded atomically to immutable Room DB | `SEC-PERM-001` | observability | 8 | `SEC-PERM-001` passes with every side-effecting Tool call retaining correlation, actor, Tool, redacted parameters, result, authorization decision, and durable audit evidence | A storage or logging failure could leave incomplete attribution; compensating controls are atomic audit persistence, correlation requirements, fail-closed evidence handling, and no successful completion claim without required audit evidence |
| **TM-010** | Repudiation | Immutable permission grant logs (`PermissionModel.md`) | `PermissionManager.decide` | User grants are written atomically to `permission_audit_log` | `SEC-PERM-002` | observability | 8 | `SEC-PERM-002` passes with grant, denial, expiry, revocation, cancellation, and final disposition retained in the durable permission audit record | Database corruption or unauthorized audit mutation could weaken proof of user intent; compensating controls are app-private durable storage, append-only audit semantics, correlation, and PermissionModel fail-closed behavior |
...
| **TM-012** | Info Disclosure | Isolated bounds and signature perms (`SECURITY_MODEL.md`) | Android IPC boundary | Block external binders; expose only bound services with sig perms | `SEC-SBX-001` | sandbox | 6 | `SEC-SBX-001` passes on an API 34 emulator with unauthorized external binders and content-provider exposure rejected while app-private data remains inaccessible | A privileged or rooted device may bypass Android IPC isolation; compensating controls are signature permissions, app-private storage, Sandbox containment, and no exposed content-provider boundary |
| **TM-013** | Info Disclosure | ProGuard/R8 symbol stripping (`standards/Security-Standard.md`) | Build Pipeline | Strip log calls containing sensitive keys in release builds | `SEC-SECRET-001` | provider | 7 | `SEC-SECRET-001` passes on an API 34 emulator and release-build logs contain no provider API key or secret reference | A build or logging configuration error could expose key material; compensating controls are secret redaction, no-secret logging rules, secure provider storage, and release-build inspection |
| **TM-014** | Info Disclosure | Log level gating and prompt sanitization (`ProviderSDK.md`) | `SanitizingHttpClient` | Redact API keys and prompt bodies from standard Logcat | `SEC-SECRET-001` | provider | 7 | `SEC-SECRET-001` passes on an API 34 emulator with API keys and sensitive prompt/response bodies absent from Logcat, telemetry, evidence, and artifacts | Misconfigured debug logging or an unsanitized adapter could expose user content; compensating controls are centralized sanitization, release log-level restrictions, redaction, and provider isolation |
| **TM-015** | Info Disclosure | Disable app backup manifest configuration (`specs/WORKSPACE.md`) | Android Manifest | `allowBackup=false` prevents adb extraction of workspace data | `SEC-SBX-001` | sandbox | 6 | `SEC-SBX-001` passes on an API 34 emulator with backup extraction disabled and workspace data inaccessible through unauthorized backup or app-to-app access | A rooted or privileged backup path may bypass manifest protections; compensating controls are app-private storage, signature permissions, encryption, and Sandbox/Workspace authorization |
...
| **TM-023** | Privilege Elev | User-approved scope review at install (`PermissionModel.md`) | `PluginManager.install` | Only user-granted scopes are accessible; others blocked | `SEC-PLUGIN-001` | plugin | 6 | `SEC-PLUGIN-001` passes on an API 34 emulator with undeclared or unapproved plugin scopes blocked, audited, and non-executable | A malicious plugin or compromised host may attempt scope confusion; compensating controls are signature verification, registry validation, PermissionModel checks, Sandbox containment, and capability blocking |
| **TM-024** | Privilege Elev | Per-step individual permission validation (`PermissionModel.md`) | `PermissionManager.check` | No implicit inheritance across tool chains; block with `NXR-2003` | `SEC-PERM-001` | runtime | 6 | `SEC-PERM-001` passes on an API 34 emulator with every chained Tool call individually authorized and a denied later step producing no side effect | A runtime or adapter bypass could carry an earlier grant into a later Tool call; compensating controls are per-call PermissionModel evaluation, default deny, audit, and Tool-side-effect gating |
| **TM-025** | Privilege Elev | Schema-enforced tool call validation (`Tool-API.md`) | `ToolManager.resolve` | Unregistered tool names are rejected with `NXR-2001` | `SEC-PERM-001` | runtime | 7 | `SEC-PERM-001` passes on an API 34 emulator with provider-generated unregistered or schema-invalid Tool calls rejected before authorization or execution and no side effect committed | A malicious or compromised provider response could carry injection data before validation; compensating controls are treating provider output as data, registry/schema validation, PermissionModel authorization, and no direct model-to-process path |
...
| **TM-031** | Spoofing | Mutual TLS 1.3 with pinned pipeKey certificates + DLP egress scan (`specs/PIPES.md` §5, §8; `NFR-SEC-014`) | `PipeTransport` | No CA or self-signed fallback; identity mismatch drops connection; deterministic secret-material blocking on outbound payloads blocks credential leak (`NFR-SEC-013`) | `SEC-NET-001` | network | 7 | SEC-NET-001 passes on API 34 emulator | mTLS with pinned certs; cert rotation requires re-pair; compensating: pipeKey revocation |
...
| Tampering | 8 | 0 | 7 | 1 |
| Repudiation | 4 | 0 | 4 | 0 |
| Information Disclosure | 11 | 0 | 10 | 1 |
| Denial of Service | 10 | 0 | 10 | 0 |
| Elevation of Privilege | 6 | 0 | 6 | 0 |
| **Total** | **47** | **0** | **45** | **2** |
