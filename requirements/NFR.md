...
| NFR-SEC-013 | Egress data-loss prevention | Secret-material transmission blocking | Configured credentials, API keys, and `SecureKeyStore` contents are blocked from transmission to any endpoint except their declared service; general full-body policy scanning is not required. Egress remains enforced at the boundary: guest processes cannot open direct outbound sockets; all traffic is forced through the workspace egress proxy (`docs/SANDBOX_DEPTH.md` §2.4), which may terminate guest TLS with a workspace-scoped CA to enforce secret-material blocking and fails closed on bypass attempts |
