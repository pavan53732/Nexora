...
| NFR-REL-003 | Error recovery | Automatic retry (configurable) | Exponential backoff, max 3 attempts; each delay = `base × 2^attempt × random(0.5…1.5)` (jitter); retry across tool invocations, sub-agent delegation, and provider reconnects per [AUTONOMY_STABILITY.md §10](../specs/AUTONOMY_STABILITY.md#10-semantic-progress--anti-replay) and [ADR-0009](docs/adr/ADR-0009-Adaptive-Autonomy-And-Persistence.md) Decision #8. |
