...
| | `NXR-2003` (Authorization Denied) | ToolInvocation status → `PENDING_AUTHORIZATION` for an effective `ASK` decision. On `USER_DENIED`, or when an approval transaction expires with its exact subreason still OPEN/DEFERRED, the Tool returns `ToolResult.Error` without side effects; the owning Task commits `WaitingApproval → Failed`, while the participating Agent may project `WaitingApproval → Paused` under DEC-35. Approval is never represented as a `ToolResult` variant, and a later attempt requires a new authorization transaction. |
