...
| `resumeExecution` | Interrupted or escalated nonterminal Execution retains/returns existing `RUNNING` as a non-terminal/resumable projection only; a Task may remain `BlockedAwaitingInput` until `resolveEscalation` | Same-ID projection with higher version | Corrupt checkpoint (`NXR-1004`), stale version, terminal target | Idempotency key required | Same `executionId`/`correlationId`; checkpoint required; while Task `BlockedAwaitingInput` for unresolved `UNKNOWN_COMPLETION`, no Tool execution or automatic replay is authorized | `IT-LC-001..005`, `IT-LC-018/020` |
