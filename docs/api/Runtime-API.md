...
| `resumeExecution` | Interrupted or explicitly clarification-blocked nonterminal Execution retains/returns existing `RUNNING` as a non-terminal/resumable projection only; exhausted `UNKNOWN_COMPLETION` uses the existing non-success path rather than `BlockedAwaitingInput` | Same-ID projection with higher version | Corrupt checkpoint (`NXR-1004`), stale version, terminal target | Idempotency key required | Same `executionId`/`correlationId`; checkpoint required; unresolved `UNKNOWN_COMPLETION` is not executed or replayed automatically; bounded exhaustion commits existing Task/Execution failure effects | `IT-LC-001..005`, `IT-LC-011`, `IT-LC-018/020` |
