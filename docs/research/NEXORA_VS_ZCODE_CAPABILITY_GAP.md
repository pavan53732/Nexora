...
> ⚠ **HISTORICAL SNAPSHOT — DATA MAY BE STALE.** This document was written on 2026-08-05 against commit `d7b670d`. The current HEAD (`524680a`, 2026-08-06) adds S5 (multi-instance pipes), S6 (reasoning effort scale), S3-E (lifecycle state machines), and restores docs/adr/docs/api/testing/cases. Tool count (316→343), feature count (FEAT-033→034), state-machine count (5→9), spec count (11→12), and FR count (95→100+) have all changed since this analysis. **Treat all metrics as pre-S5/S6 values unless a current-state banner confirms otherwise.**
...
- A capability is reported as **Fully Specified (not implemented)** only when the specification defines interfaces, data models, lifecycle states, requirements IDs, and phase mappings.
...
| 10 | **Edit history** | **Partially Specified** | `architecture/CONVERSATION_CHECKPOINTS.md` and `specs/CONVERSATION_CHECKPOINTS.md` now define non-destructive conversation branching from immutable conversation checkpoints; `FR.md`: FR-M012 (file history); `specs/FILE_SYSTEM.md`: file versioning; `docs/SANDBOX_DEPTH.md`: workspace snapshots and rollback | ZCode's "Edit History" is conversation-level rollback (`Chat Versioning` with checkpoints). Nexora's file version history (`FR-M012`) is per-file snapshot/revert within sandbox. Workspace-level rollback (`FR-S013`) is specified as full-workspace snapshot/restore. Conversation rollback is now semantically specified as a non-destructive branch, but implementation and final persistence/UI phase remain planned; file and workspace rollback remain separate operations. |
