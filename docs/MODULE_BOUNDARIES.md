...
| **runtime** | `com.nexora.app.runtime` | Agent loop, planner, executor, context builder, token budgeting, skill registry | `AgentLoop`, `Planner`, `Executor`, `ContextBuilder`, `TokenBudget`, `Skill`, `SkillRegistry`, `NexoraEvent` | `tools`, `providers`, `memory`, `agents`, `workflows`, `storage`, `security`, `shared` | `ui`, `application`, `sandbox`, `services` |
