...
| **runtime** | `com.nexora.app.runtime` | Agent loop, planner, executor, context builder, token budgeting | `AgentLoop`, `Planner`, `Executor`, `ContextBuilder`, `TokenBudget`, `NexoraEvent` | `tools`, `providers`, `memory`, `agents`, `workflows`, `storage`, `security`, `shared` | `ui`, `application`, `sandbox`, `services` |
...
| **security** | `com.nexora.app.security` | PermissionManager, SecureKeyStore, audit logging | `PermissionManager`, `PermissionScope`, `PermissionDecision`, `SecureKeyStore`, `AuditLogger` | `storage`, `shared` | `ui`, `application`, `runtime`, `tools`, `sandbox`, `providers`, `memory`, `agents`, `plugins`, `workflows`, `services` |
