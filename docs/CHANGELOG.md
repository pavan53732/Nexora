...
- Governance resolution: qualified the Product Vision Android-first wording without unsupported market exclusivity (DEC-26); replaced undefined `Platform Infrastructure` coverage/test owner labels with concrete existing authorities (DEC-27); and normalized ROADMAP navigation/plugin terminology to documented behavior and interface contracts (DEC-28). The execution-failure class-to-error/lifecycle binding remains explicitly OPEN/DEFERRED; no unsupported error or lifecycle mapping was invented.
