...
### Added
- S13 — permission and lifecycle governance corrections: PermissionScopeRegistry + 18-scope model alignment; unknown-scope denial via NXR-2003 + DenialReason; per-scope ASK audit outcomes with ApprovalTransactionResult; classifier gate relocated after all scopes pass; PolicySource attribution for every decision; Session expiration guards (3 specific rows); deterministic Execution resume identity (same executionId for checkpoint, new for terminal retry); Decisi...[truncated]
