...
At the current published HEAD, the active engineering contract set has no unresolved approval-expiry or cloud-only AI-provider scope boundary. DEC-36 classifies an approval transaction that expires before a valid authorization outcome as the existing `NXR-2003 / POLICY_DENIAL` authorization-gate denial, while preserving expiry as a distinct audit, trace, notification, and activity-feed outcome. Most product, architecture, ownership, lifecycle, protocol, security, and terminology decisions in this inventory are resolved or explicitly bounded downstream. Two requirement-authority identity collisions remain active: `NFR-PERF-006` has distinct battery-impact and execution-mode definitions, and `NFR-REL-004` has distinct persistence and bounded-progress definitions. These require requirements-authority correction before complete unique traceability can be claimed. The repository also contains historical unresolved wording, bounded downstream choices, and planned evidence obligations; these categories are not interchangeable with an active owner decision.
...
- `NFR-PERF-006` and `NFR-REL-004` remain requirement-authority identity collisions. Their distinct definitions cannot share one canonical ledger row; no replacement identifiers are inferred. Downstream mappings and validation projections must remain provisional until the requirements authority resolves the identity boundary.
...
The pre-source audit identified and resolved the documentation contracts required before Android source bootstrap. DEC-37 fixes the current API 34 min/compile/target baseline. DEC-38 separates minimal-foundation APK gates from the 80 MB architecture-specific Full Environment delivery gate. DEC-39 is retained as historical local-GGUF boundary authority but its active scope is superseded by DEC-41. DEC-40 permits public-interface dependencies along the canonical module graph and reserves EventBus for event transport. DEC-41 fixes cloud/external-only AI providers and cloud-only degradation. DEC-42 retires the optional local TFLite classifier and fixes the no-local-AI-model boundary.
