...
- Execution-failure responsibility and class binding are resolved by DEC-29: `errors/ERROR_CODES.md` owns error identity and shared recovery metadata, the owning lifecycle owns legal lifecycle effect, and the operation owner owns idempotency and retry conditions. Transient, permanent, and escalation classifications do not create new errors or states; legal Task/Execution effects remain those of the existing canonical state machines.
...
The Product Vision positioning, requirement owner labels, ROADMAP terminology boundaries, execution-failure class binding, agent-loop liveness boundaries, BranchLineage operational policy, and workflow scope are resolved by DEC-26 through DEC-32. DEC-31 selects 30-day superseded checkpoint retention, a 100-checkpoint per-workspace quota, and daily idempotent cleanup; concrete storage encoding, transaction mechanism, and transport remain downstream implementation choices. Concrete Conversation metadata fields and schema remain downstream under DEC-24.
