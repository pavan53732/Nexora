...
- Task approval denial and external-wait expiry are resolved by DEC-30: explicit denial fails the Task through existing `NXR-2003`/`USER_DENIED`; expired approval uses existing `NXR-2003`/`POLICY_DENIAL`; unanswered `BlockedAwaitingInput` fails on the effective deadline; no new state is created.
