...
At the current published HEAD, the active engineering contract set has one preserved error-identity boundary marked OPEN/DEFERRED: the subreason for an expired approval transaction. All other product, architecture, ownership, lifecycle, protocol, security, and terminology decisions in this inventory are resolved or explicitly bounded downstream. The repository also contains historical unresolved wording, bounded downstream choices, and planned evidence obligations; these categories are not interchangeable with an active owner decision.

The approval-expiry boundary is narrow: DEC-30 and DEC-35 establish the Task `WaitingApproval → Failed` effect and no automatic retry or side effect, but the canonical NXR-2003 taxonomy defines `POLICY_DENIAL` for effective policy DENY and does not establish a distinct approval-expired subreason. The identity remains OPEN/DEFERRED pending compatible canonical evidence; no new subreason or error code is defined here.

The historical `OWNER DECISION REQUIRED / UNRESOLVED` passages in DEC-13 and DEC-14 are preserved decision records. Their formerly open boundaries are superseded or narrowed by DEC-15 through DEC-24 and DEC-25 through DEC-35: Session–Conversation ownership, relationship identity/representation/cardinality/lifecycle/continuation, BranchLineage ownership and operational policy, checkpoint metadata semantics and retention, no internal credit/cost gating, Android-first positioning, requirement-owner taxonomy, ROADMAP terminology, execution-failure binding, agent-loop liveness, Task liveness error identities, background terminal parent binding, approval-denial composition, and workflow scope are now represented by later canonical decisions. DEC files remain immutable historical authority and are not rewritten.
...
Other unresolved values allowed by this inventory are local `UNKNOWN` or `UNAVAILABLE` data states explicitly defined by a contract, such as Tool `UNKNOWN_COMPLETION` pending reconciliation or provider health before its first check. They are runtime states or evidence classifications, not owner decisions. The separate approval-expiry subreason boundary is recorded above as OPEN/DEFERRED.
...
The Product Vision positioning, requirement owner labels, ROADMAP terminology boundaries, execution-failure class binding, agent-loop liveness boundaries, Task liveness error identities, background terminal liveness, approval-denial composition, BranchLineage operational policy, and workflow scope are resolved by DEC-26 through DEC-35. The approval-expiry subreason remains separately OPEN/DEFERRED as recorded in the active register. DEC-31 selects 30-day superseded checkpoint retention, a 100-checkpoint per-workspace quota, and daily idempotent cleanup; DEC-34 selects parent binding, deadline inheritance, and orphan reconciliation for autonomous background terminals; concrete storage encoding, transaction mechanism, process cleanup, and transport remain downstream implementation choices. Concrete Conversation metadata fields and schema remain downstream under DEC-24.
...
- Task approval denial and external-wait state effects are resolved by DEC-30/DEC-35: explicit denial fails the Task through existing `NXR-2003`/`USER_DENIED`; an expired approval also fails the Task without automatic approval or retry, but its exact NXR-2003 subreason remains OPEN/DEFERRED because `POLICY_DENIAL` is canonically defined for effective policy DENY; unanswered `BlockedAwaitingInput` fails on the effective deadline; no new state is created.
