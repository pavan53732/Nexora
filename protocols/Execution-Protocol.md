...
5. **ESCALATION**: `requestEscalation(question)` may place the owning Task in nonterminal `BlockedAwaitingInput` for human clarification or exhausted `UNKNOWN_COMPLETION` reconciliation. The associated Execution retains the existing `RUNNING` status because no blocked or suspended ExecutionStatus exists; this is an existing non-terminal/resumable projection only and authorizes no further Tool execution or automatic replay while awaiting the answer. User answer injected via `resolveEscalation` resumes the existing `BlockedAwaitingInput` path; `executionId`/`correlationId` are retained, `version` increments, and `escalationPayload` (stored on checkpoint) is cleared after resume.
