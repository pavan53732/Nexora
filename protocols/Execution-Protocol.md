...
5. **ESCALATION**: `requestEscalation(question)` may place the owning Task in nonterminal `BlockedAwaitingInput` only for explicit human clarification or capability gaps. Exhausted `UNKNOWN_COMPLETION` reconciliation does not use this path: the child remains unresolved, evidence is retained, and existing Task/Execution non-success effects are committed automatically. While a Task is explicitly `BlockedAwaitingInput`, the associated Execution retains the existing `RUNNING` status because no blocked or suspended ExecutionStatus exists; this is an existing non-terminal/resumable projection only and authorizes no further Tool execution or automatic replay while awaiting the answer. User answer injected via `resolveEscalation` resumes the existing `BlockedAwaitingInput` path; `executionId`/`correlationId` are retained, `version` increments, and `escalationPayload` (stored on checkpoint) is cleared after resume.
