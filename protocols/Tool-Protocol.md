...
> **Mapping rule:** Every MCP primitive (`mcp_call_tool`, `mcp_read_resource`, `mcp_get_prompt`) produces a standard `ToolExecutionMessage` (same `correlationId`, `workspaceId`, `toolCallId`, `limits`) and returns through the standard `ToolResult.Success` / `.Error` pipeline; authorization requirements surface as `ToolInvocation` status `PENDING_AUTHORIZATION`, not a `ToolResult` variant.
