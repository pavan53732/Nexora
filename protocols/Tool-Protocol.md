...
2. **Cross-layer denial projection (DEC-35):** On explicit `USER_DENIED`, or on an approval transaction that expires with its exact subreason still OPEN/DEFERRED, the Tool boundary returns the existing `NXR-2003` denial result without a side effect and does not publish successful `ToolExecuted` semantics. The owning Task commits `WaitingApproval → Failed`; a participating Agent may independently publish `AgentStatusChanged` to `Paused`. A later attempt requires a new approval transaction.
